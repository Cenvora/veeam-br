...
def _get_kwargs(
    *,
    ...
    json_malware_status_filter: list[str] | Unset = UNSET
    if not isinstance(malware_status_filter, Unset):
        json_malware_status_filter = []
        for malware_status_filter_item_data in malware_status_filter:
            malware_status_filter_item = malware_status_filter_item_data.value
            json_malware_status_filter.append(malware_status_filter_item)
    ...
    json_exclude_snapshot_type_filter: list[int] | Unset = UNSET
    if not isinstance(exclude_snapshot_type_filter, Unset):
        json_exclude_snapshot_type_filter = exclude_snapshot_type_filter

    params["excludeSnapshotTypeFilter"] = json_exclude_snapshot_type_filter

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/v1/restorePoints",
        "params": params,
    }
...
def sync_detailed(
    *,
    ...
    """Get All Restore Points
     ...
     The HTTP GET request to the `/api/v1/restorePoints` endpoint gets an array of all restore points
    created on the backup server.<p>**Available to**&#58; Backup Administrator, Backup Operator, Restore
    Operator, Backup Viewer. Also available to custom roles that have backup or restore permissions.</p>
    ...
    Args:
        ...
        platform_name_filter (EPlatformType | Unset): Platform type.<p>`Test` is the platform of
            SureBackup content scan job (`SureBackupContentScan`) — backup verification and content
            scanning with antivirus software or YARA rules.</p>
