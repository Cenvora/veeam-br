...
from ...models.ad_domains_result import ADDomainsResult
...
) -> ADDomainsResult | Error | None:
    if response.status_code == 200:
        response_200 = ADDomainsResult.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ADDomainsResult | Error]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """Get Active Directory Domains
     ...
     The HTTP GET request to the `/api/v1/adDomains` endpoint gets an array of Active Directory domains
    that are added to the backup server. <p>**Available to**&#58; Backup Administrator.</p>
