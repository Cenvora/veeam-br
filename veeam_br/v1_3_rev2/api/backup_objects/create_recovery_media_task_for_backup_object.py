...
def sync_detailed(
    ...
    *,
    ...
    """Create Recovery Media Task for Backup Object
     ...
     The HTTP POST request to the `/api/v1/backupObjects/{id}/createRecoveryMedia` endpoint starts an
    asynchronous task that creates Veeam Recovery Media for a backup created by Veeam Agent for
    Microsoft Windows. The endpoint returns a task that you can poll through the [Get
