...
def sync_detailed(
    *,
    ...
    """Start Instant FCD Recovery
     ...
     The HTTP POST request to the `/api/v1/restore/instantRecovery/vSphere/fcd` endpoint starts Instant
    FCD Recovery from the restore point to the destination cluster.<p>Specify the destination cluster in
    the `destinationCluster` parameter of the request body as a model of the VMware vSphere object. For
    details on how to get the cluster model, see [Get Inventory Objects](Inventory-
    Browser#operation/GetInventoryObjects).<p>**Available to**&#58; Backup Administrator, Restore
