...
def sync_detailed(
    *,
    ...
    """Get Credentials for Delegated Restore of Microsoft Entra ID Items
     ...
     The HTTP POST request to the `/api/v1/restore/entraId/tenant/deviceCode/state` endpoint gets
    credentials that are required for delegated restore of Microsoft Entra ID items. Before you request
    the credentials, obtain a user code using the [Get User Code for Delegated Restore of Microsoft
    Entra ID Items](Restore#operation/GetEntraIdTenantRestoreDeviceCode) request.<p>You can use the
    credentials in the following requests&#58;</p> <ul><li>[Restore Microsoft Entra ID Items](Backup-
    Browser#operation/RestoreEntraIdTenantItems)</li> <li>[Restore Microsoft Entra ID Item
    Properties](Backup-Browser#operation/RestoreEntraIdTenantItemAttributes)</li> </ul><p>**Available
