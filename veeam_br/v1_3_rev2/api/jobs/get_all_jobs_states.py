...
def _get_kwargs(
    *,
    ...
    json_after_job_id_filter: str | Unset = UNSET
    if not isinstance(after_job_id_filter, Unset):
        json_after_job_id_filter = str(after_job_id_filter)
    params["afterJobIdFilter"] = json_after_job_id_filter

    params["afterJobNameFilter"] = after_job_name_filter
...
def sync_detailed(
    *,
    ...
    """Get All Job States
     ...
     The HTTP GET request to the `/api/v1/jobs/states` endpoint gets an array of all job states. The
    states include brief job information that you can also find under the **Jobs** node in the Veeam
    Backup & Replication console.<p>**Available to**&#58; Backup Administrator, Backup Operator, Restore
    Operator, Backup Viewer, Security Administrator. Also available to custom roles that have backup or
    ...
    kwargs = _get_kwargs(
        ...
        after_job_id_filter=after_job_id_filter,
        after_job_name_filter=after_job_name_filter,
