...
def sync_detailed(
    ...
    *,
    ...
    """Get Restore Points of Microsoft Entra ID Tenant
     ...
     The HTTP POST request to the `/api/v1/backupBrowser/entraIdTenant/{backupId}/restorePoints` endpoint
    gets an array of restore points available in a Microsoft Entra ID tenant backup that has the
    specified `backupId`. This request helps you find the restore point from which you want to restore
    tenant items.<p>**Available to**&#58; Backup Administrator, Restore Operator. Also available to
    custom roles that have restore permissions.</p>

    Args:
        backup_id (UUID):
        x_api_version (str):  Default: '1.3-rev2'.
        body (EntraIdTenantRestorePointSpec): Restore point settings.
