...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/v1/backupBrowser/entraIdTenant/{backup_id}/browse".format(
...
def sync_detailed(
    ...
    *,
    ...
    """Get Microsoft Entra ID Items
     ...
     The HTTP POST request to the `/api/v1/backupBrowser/entraIdTenant/{backupId}/browse` endpoint
    browses Microsoft Entra ID items available in a backup that has the specified `backupId`. Use this
    request to find the items that you want to restore. In the request body, you must specify the item
    type.<p>**Available to**&#58; Backup Administrator, Restore Operator. Also available to custom roles
    that have restore permissions.</p>
