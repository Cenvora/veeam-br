...
def sync_detailed(
    ...
    *,
    ...
    """Validate Microsoft Entra ID Items
     ...
     The HTTP POST request to the `/api/v1/backupBrowser/entraIdTenant/{sessionId}/validate` endpoint
    validates that Microsoft Entra ID items specified in the request body are available in a specified
    restore point.<p>In the response body, you receive an array of items that are missing in the restore
    point. To browse restore points that are available for each of the missing items, run the [Get
    Restore Points of Microsoft Entra ID Item](Backup-
    Browser#operation/BrowseEntraIdTenantRestorePoints) request.</p><p>**Available to**&#58; Backup
    ...
    Args:
        ...
        body (EntraIdTenantValidateSpec): Settings for validating that items exist in a restore
            point.
