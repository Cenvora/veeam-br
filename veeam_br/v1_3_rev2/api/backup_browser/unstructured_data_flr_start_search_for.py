...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/v1/backupBrowser/flr/unstructuredData/{session_id}/search".format(
...
def sync_detailed(
    ...
    *,
    ...
    endpoint creates a search task to find required file system items (drives, folders, files and links)
    available in a restore session that has the specified `sessionId`. <p>To get the search results, run
    the [Browse Search Results in Unstructured Data Mount Point](Backup-
    Browser#operation/UnstructuredDataFlrBrowseSearchResult) request. <p>**Available to**&#58; Backup
