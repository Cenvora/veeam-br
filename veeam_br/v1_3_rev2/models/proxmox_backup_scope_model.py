...
if TYPE_CHECKING:
    from ..models.proxmox_object_model import ProxmoxObjectModel


T = TypeVar("T", bound="ProxmoxBackupScopeModel")


@_attrs_define
class ProxmoxBackupScopeModel:
    """Proxmox VE backup scope.

    Attributes:
        type_ (EInventoryScopeWorkloadType): Workload type.
        inventory_objects (list[ProxmoxObjectModel] | Unset): Array of Proxmox VE inventory objects.
    """

    type_: EInventoryScopeWorkloadType
    inventory_objects: list[ProxmoxObjectModel] | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        if _inventory_objects is not UNSET:
            ...
            for inventory_objects_item_data in _inventory_objects:
                inventory_objects_item = ProxmoxObjectModel.from_dict(inventory_objects_item_data)

                inventory_objects.append(inventory_objects_item)

        proxmox_backup_scope_model = cls(
            type_=type_,
            inventory_objects=inventory_objects,
        )

        proxmox_backup_scope_model.additional_properties = d
        return proxmox_backup_scope_model
