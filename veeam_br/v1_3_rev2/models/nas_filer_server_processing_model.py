...
class NASFilerServerProcessingModel:
    """NAS filer processing settings.

    Attributes:
        cache_repository_id (UUID): ID of a backup repository that is used as a cache repository for the server. To get
            the ID, run the [Get All Repositories](Repositories#operation/GetAllRepositories) request.
        ...
        native_change_tracking_enabled (bool | Unset): If `true`, Veeam Backup & Replication will use file change
            tracking technology provided by the storage system manufacturer.
    """
