...
T = TypeVar("T", bound="GuestInteractionProxiesSettingsModel")


@_attrs_define
class GuestInteractionProxiesSettingsModel:
    """Guest interaction proxy used to deploy the runtime process on the VM guest OS.

    Attributes:
        auto_select_enabled (bool): If `true`, Veeam Backup & Replication automatically selects the guest interaction
            proxy. Default: True.
        proxy_ids (list[UUID] | Unset): Array of proxies specified explicitly. The array must contain servers registered
            as guest interaction proxies in the backup infrastructure.
    """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        guest_interaction_proxies_settings_model = cls(
            auto_select_enabled=auto_select_enabled,
            proxy_ids=proxy_ids,
        )

        guest_interaction_proxies_settings_model.additional_properties = d
        return guest_interaction_proxies_settings_model
