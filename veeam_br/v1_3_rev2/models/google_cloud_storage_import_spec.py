from __future__ import annotations

from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..models.e_repository_type import ERepositoryType
from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.google_cloud_storage_account_import_model import GoogleCloudStorageAccountImportModel
    from ..models.google_cloud_storage_bucket_model import GoogleCloudStorageBucketModel
    from ..models.mount_servers_settings_import_spec import MountServersSettingsImportSpec


T = TypeVar("T", bound="GoogleCloudStorageImportSpec")


@_attrs_define
class GoogleCloudStorageImportSpec:
    """Import settings for Google Cloud storage.

    Attributes:
        name (str): Name of the object storage repository.
        description (str): Description of the object storage repository.
        unique_id (str): Unique ID that identifies the object storage repository.
        type_ (ERepositoryType): Repository type.
        account (GoogleCloudStorageAccountImportModel): Account used to access the Google Cloud storage.
        bucket (GoogleCloudStorageBucketModel): Google Cloud object storage bucket where backup data is stored.
        enable_task_limit (bool | Unset): If `true`, the maximum number of concurrent tasks is limited.
        max_task_count (int | Unset): Maximum number of concurrent tasks.
        mount_server (MountServersSettingsImportSpec | Unset): Import settings for mount servers.
    """

    name: str
    description: str
    unique_id: str
    type_: ERepositoryType
    account: GoogleCloudStorageAccountImportModel
    bucket: GoogleCloudStorageBucketModel
    enable_task_limit: bool | Unset = UNSET
    max_task_count: int | Unset = UNSET
    mount_server: MountServersSettingsImportSpec | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        name = self.name

        description = self.description

        unique_id = self.unique_id

        type_ = self.type_.value

        account = self.account.to_dict()

        bucket = self.bucket.to_dict()

        enable_task_limit = self.enable_task_limit

        max_task_count = self.max_task_count

        mount_server: dict[str, Any] | Unset = UNSET
        if not isinstance(self.mount_server, Unset):
            mount_server = self.mount_server.to_dict()

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "name": name,
                "description": description,
                "uniqueId": unique_id,
                "type": type_,
                "account": account,
                "bucket": bucket,
            }
        )
        if enable_task_limit is not UNSET:
            field_dict["enableTaskLimit"] = enable_task_limit
        if max_task_count is not UNSET:
            field_dict["maxTaskCount"] = max_task_count
        if mount_server is not UNSET:
            field_dict["mountServer"] = mount_server

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.google_cloud_storage_account_import_model import GoogleCloudStorageAccountImportModel
        from ..models.google_cloud_storage_bucket_model import GoogleCloudStorageBucketModel
        from ..models.mount_servers_settings_import_spec import MountServersSettingsImportSpec

        d = dict(src_dict)
        name = d.pop("name")

        description = d.pop("description")

        unique_id = d.pop("uniqueId")

        type_ = ERepositoryType(d.pop("type"))

        account = GoogleCloudStorageAccountImportModel.from_dict(d.pop("account"))

        bucket = GoogleCloudStorageBucketModel.from_dict(d.pop("bucket"))

        enable_task_limit = d.pop("enableTaskLimit", UNSET)

        max_task_count = d.pop("maxTaskCount", UNSET)

        _mount_server = d.pop("mountServer", UNSET)
        mount_server: MountServersSettingsImportSpec | Unset
        if isinstance(_mount_server, Unset):
            mount_server = UNSET
        else:
            mount_server = MountServersSettingsImportSpec.from_dict(_mount_server)

        google_cloud_storage_import_spec = cls(
            name=name,
            description=description,
            unique_id=unique_id,
            type_=type_,
            account=account,
            bucket=bucket,
            enable_task_limit=enable_task_limit,
            max_task_count=max_task_count,
            mount_server=mount_server,
        )

        google_cloud_storage_import_spec.additional_properties = d
        return google_cloud_storage_import_spec

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
