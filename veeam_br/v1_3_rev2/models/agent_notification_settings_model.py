from __future__ import annotations

from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.agent_backup_monitoring_settings_model import AgentBackupMonitoringSettingsModel
    from ..models.email_notification_settings_model import EmailNotificationSettingsModel


T = TypeVar("T", bound="AgentNotificationSettingsModel")


@_attrs_define
class AgentNotificationSettingsModel:
    """Notification settings.

    Attributes:
        send_snmp_notifications (bool | Unset): If `true`, SNMP notifications are enabled for this job.
        email_notifications (EmailNotificationSettingsModel | Unset): Email notification settings for the job.
        backup_monitoring (AgentBackupMonitoringSettingsModel | Unset): Backup monitoring settings. If specified, a
            warning notification is sent when the agent has not reported any successful backups within the specified period.
    """

    send_snmp_notifications: bool | Unset = UNSET
    email_notifications: EmailNotificationSettingsModel | Unset = UNSET
    backup_monitoring: AgentBackupMonitoringSettingsModel | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        send_snmp_notifications = self.send_snmp_notifications

        email_notifications: dict[str, Any] | Unset = UNSET
        if not isinstance(self.email_notifications, Unset):
            email_notifications = self.email_notifications.to_dict()

        backup_monitoring: dict[str, Any] | Unset = UNSET
        if not isinstance(self.backup_monitoring, Unset):
            backup_monitoring = self.backup_monitoring.to_dict()

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if send_snmp_notifications is not UNSET:
            field_dict["sendSNMPNotifications"] = send_snmp_notifications
        if email_notifications is not UNSET:
            field_dict["emailNotifications"] = email_notifications
        if backup_monitoring is not UNSET:
            field_dict["backupMonitoring"] = backup_monitoring

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.agent_backup_monitoring_settings_model import AgentBackupMonitoringSettingsModel
        from ..models.email_notification_settings_model import EmailNotificationSettingsModel

        d = dict(src_dict)
        send_snmp_notifications = d.pop("sendSNMPNotifications", UNSET)

        _email_notifications = d.pop("emailNotifications", UNSET)
        email_notifications: EmailNotificationSettingsModel | Unset
        if isinstance(_email_notifications, Unset):
            email_notifications = UNSET
        else:
            email_notifications = EmailNotificationSettingsModel.from_dict(_email_notifications)

        _backup_monitoring = d.pop("backupMonitoring", UNSET)
        backup_monitoring: AgentBackupMonitoringSettingsModel | Unset
        if isinstance(_backup_monitoring, Unset):
            backup_monitoring = UNSET
        else:
            backup_monitoring = AgentBackupMonitoringSettingsModel.from_dict(_backup_monitoring)

        agent_notification_settings_model = cls(
            send_snmp_notifications=send_snmp_notifications,
            email_notifications=email_notifications,
            backup_monitoring=backup_monitoring,
        )

        agent_notification_settings_model.additional_properties = d
        return agent_notification_settings_model

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
