...
if TYPE_CHECKING:
    from ..models.nutanix_object_model import NutanixObjectModel


T = TypeVar("T", bound="NutanixBackupScopeModel")


@_attrs_define
class NutanixBackupScopeModel:
    """Nutanix AHV backup scope.

    Attributes:
        type_ (EInventoryScopeWorkloadType): Workload type.
        inventory_objects (list[NutanixObjectModel] | Unset): Array of Nutanix AHV inventory objects.
    """

    type_: EInventoryScopeWorkloadType
    inventory_objects: list[NutanixObjectModel] | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        if _inventory_objects is not UNSET:
            ...
            for inventory_objects_item_data in _inventory_objects:
                inventory_objects_item = NutanixObjectModel.from_dict(inventory_objects_item_data)

                inventory_objects.append(inventory_objects_item)

        nutanix_backup_scope_model = cls(
            type_=type_,
            inventory_objects=inventory_objects,
        )

        nutanix_backup_scope_model.additional_properties = d
        return nutanix_backup_scope_model
